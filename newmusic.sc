...
~mksustainedelement = {
	...
	//hvp.postln;
	{
		|freq=1.0,amp=0.0,attack=20,decay=20,gate=1|
		//hva.postln;
		Normalizer.ar(
			Mix.ar( SinOsc.ar(freq * hvp, mul: hva) ) / n,
		)!2 * EnvGen.kr(
			Env.adsr(attack,attack,0.5,decay),
			//Env.new([0,0.5,0],[attack,decay]),
			doneAction: 2
		)!2 * amp
	}
...
~playsustainedelm = {|elm,amp=0.0,freq=440.0,attack=20.0,decay=20.0,n=100| ~mksustainedelement.(~elem[elm],n:n).play(s,args:[\amp,amp,\freq,freq,\attack,attack,\decay,decay]) };
...
// ~longwood = 
~woodplay.(waitTime: 120, portion: 0.1, freq: 2**(14.linrand+2), l: ~wood.choose.scramble)


~woodplay.(waitTime: 10+10.linrand, freq: 4000.rand+20, l: ~wood.choose)
~woodplay.(waitTime: 10+40.linrand, portion: 0.5, freq: 40+2000.rand, l: ~wood.choose)
...
~woodplay.(waitTime: 10+100.linrand, freq: 40, l: ["H","Cl"])
~woodplay.(waitTime: 10+120.linrand, freq: 1000.rand, l: ["O","O","Fe","Fe","Fe"])
~woodplay.(waitTime: 20+40.linrand, freq: 1000.linrand, l: ~wood.choose.scramble)
...

{
	var l = ~wood.choose;
	5.do{|x| 
		~woodplay.(waitTime: 120+60.linrand, portion: 0.9, freq: (x+1) * 114.0, l: l)
	}
}.()
...
~woodplay.(waitTime: 600.rand, portion: 1.0.rand , freq: 220*(1+10.rand), l: [["Fe","S","Cl","H","C"].choose])
	...
	// repeatedly apply
~woodplay.(waitTime: 10+40.linrand, portion: 0.9, freq: 299*(15.linrand+1), l: ["C","O","O","H"])
...
~dnotes = Array.newClear(128);    // array has one slot per possible MIDI note

~don = MIDIFunc.noteOn({ |veloc, num, chan, src|
	if(nil!=~dnotes[num],{
		~dnotes[num].do {|x| x.release(10); };
	});
	~dnotes[num] =~woodplay.(waitTime: 30.0, portion: 1.0.rand , freq: num.midicps, l: [["Fe","S","Cl","H","C"].choose])

},nil,1);

~doff = MIDIFunc.noteOff({ |veloc, num, chan, src|
    ~dnotes[num].do {|x| x.release(10); };
	~dnotes[num] = [];
},nil,1);
